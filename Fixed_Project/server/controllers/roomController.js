...
const getFeaturedRooms = async (req, res) => {
  try {
    // Always show the 8 most recently added available rooms
    const rooms = await Room.find({ status: { $ne: "Lended" } })
      .sort({ postedAt: -1 })
      .limit(8);
...
const updateRoom = async (req, res) => {
  try {
    ...
    const currentPrefs = (room.roommatePrefs && typeof room.roommatePrefs.toObject === "function")
      ? room.roommatePrefs.toObject()
      : { ...room.roommatePrefs };
